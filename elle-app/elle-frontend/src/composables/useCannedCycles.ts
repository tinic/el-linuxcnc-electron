...
// Shared HAL display state driven by cycle parameters
const { zpitch, xpitch, zpitchlabel, xpitchlabel, xpitchangle } = useHAL()
const { metric } = useSettings()

// Propagate the thread pitch parameter into the DRO pitch cells
...
// Direct exports for module-level consumers (e.g. the numpad state
// machine) that must not depend on the dialog-bound composable below.
export {
...
// Preset dialogs need the PrimeVue dialog service, which is only available
// during component setup — so useCannedCycles() must be called from setup.
// All state above is module-level (singleton) and shared across callers.
